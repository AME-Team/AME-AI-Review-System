...
1. **mainブランチとの全累積差分レビュー (`origin/main...HEAD`)**
   - コミット単位の差分評価では複数コミットを含むPRで変更全体の整合性が把握できない。本システムでは main ブランチとの全累積差分を網羅的に抽出し一貫したAIレビューを実施する。
2. **超厳格なデフォルト静的解析 (Static Circuit Breaker)**
   - 機械的に検出できる問題は静的解析（tsc / eslint / mypy / ruff /
     semgrep）で100%捕捉する。エラー時はAIレビューを自動スキップしコスト削減と明確なフィードバックを両立する。

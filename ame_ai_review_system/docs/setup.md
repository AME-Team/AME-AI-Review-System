...
| **ランナー** | GitHub-hosted `ubuntu-latest`（デフォルト）。ツールチェーン (Python 3.12 / Node 22 / uv / shellcheck / actionlint / gitleaks / claude-code / opencode-ai) は `.github/workflows/ci.yml` の `steps:` で都度セットアップされるため、ランナー側の事前準備は不要。ここで導入する `claude-code` / `opencode-ai` CLI はエンジン本体の呼び出しではなく、下記「LLM SDK」行の認証基盤・OpenCode サーバ（`opencode serve`）起動に使われる。                                                                                                                                                                                 |

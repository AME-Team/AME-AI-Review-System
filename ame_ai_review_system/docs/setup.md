...
| `show_engine_info_gate2`          | `true`     | PR レビュー (Gate 2) でエンジン・モデル・思考量のバナーを表示するか (Issue #40)。`false` で非表示 (非 Claude エンジンの budget 未強制警告も抑止)。CI ログの step `env:` エコーは Variables / Secrets で制御する。 |
